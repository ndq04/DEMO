  ...
  const handleSubmit = async (e) => {
    ...
    if (email && pass) {
      try {
        ...
        const res = await postDataApi('http://localhost:5000/api/auth/login', {
          email,
          password: pass,
        })
      ...
      toast.error('Login Failed ! Email and password fields are required.')
      loadingDispatch({
        type: 'LOADING',
        payload: false,
      })
      ...
      {showForm && (
        <div className='w-full flex-1'>
          ...
          <div className='flex items-center justify-between mt-6 mb-4'>
            <button
              type='submit'
              className='bg-[#f47121] text-white w-full p-2 text-lg mr-7 rounded-full'
            >
              Login
            </button>
            <Link
